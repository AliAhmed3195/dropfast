...
export default function SupplierOrderDetailsPage({ params }: { params: { id: string } }) {
  ...
  const updateOrderStatus = async (newStatus: string) => {
    try {
      setUpdating(true);
      const response = await fetch(`/api/supplier/orders/${params.id}/status`, {
        method: 'PATCH',
        headers: {
  ...
  if (loading) {
    return <Loading message="Loading order details..." fullScreen />;
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-2">
                {['CONFIRMED', 'PACKED', 'HANDED_TO_COURIER', 'SHIPPED', 'DELIVERED', 'CANCELLED'].map((status) => (
                  <button
                    key={status}
              ...
              <p className="text-xs text-gray-500 mt-2">
                Suppliers can update order status through the fulfillment process.
              </p>
            </div>
