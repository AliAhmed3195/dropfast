...
interface ClassicStoreTemplateProps {
  store: Store;
}

const ClassicStoreTemplate: React.FC<ClassicStoreTemplateProps> = ({ store }) => {
  const { templateConfig } = store;

  return (
    <div className="template-classic min-h-screen bg-white">
      {/* Header */}
          ...
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex items-center">
              {store.logo ? (
                ...
                <h1 className="text-2xl font-bold text-green-600">{store.name}</h1>
              )}
            </div>

            {/* Navigation */}
            <nav className="hidden md:flex space-x-8">
              <Link href={`/store/${store.id}`} className="text-gray-700 hover:text-green-600">
                Home
              </Link>
              <Link href={`/store/${store.id}/products`} className="text-gray-700 hover:text-green-600">
                Products
              </Link>
              <Link href={`/store/${store.id}/about`} className="text-gray-700 hover:text-green-600">
                About
              </Link>
              <Link href={`/store/${store.id}/contact`} className="text-gray-700 hover:text-green-600">
                Contact
              </Link>
            </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {store.featuredProducts && store.featuredProducts.length > 0 && (
          <section className="mb-16">
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${templateConfig?.gridColumns || 3} gap-6`}>
                ...
                <div key={product.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-4">
                    ...
                    <Link
                      ...
                      className="mt-3 inline-block bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition-colors"
                    >
                      View Details
                    </Link>
                  </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <p className="text-gray-400">Email: contact@{store.name.toLowerCase().replace(/\s+/g, '')}.com</p>
            </div>
          </div>
...
export default ClassicStoreTemplate;
